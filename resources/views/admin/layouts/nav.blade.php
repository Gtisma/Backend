<nav class="r-aside_inner r-gutter r-edges--left">
    <h2 class="sr">Main Navigation</h2>
    <ul class="r-none r-list"><li class="r-no-border r-mb-h">
            <a class="is-active r-list-item is-vertical-nav" href="https://adx-usr.now.sh/"><svg width="30" height="30" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-labelledby="homeAlt2IconTitle" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="miter" fill="none" color="#EB4224" class="icon icon-home">
                    <title id="homeAlt2IconTitle">Home</title>
                    <path d="M2 12L5 9.3M22 12L19 9.3M19 9.3L12 3L5 9.3M19 9.3V21H5V9.3" />
                </svg><span class="r-suffix--wider r-list-item_text">Home</span>
            </a>
        </li><li class="r-no-border r-mb-h">
            <a class=" r-list-item is-vertical-nav" href="https://adx-usr.now.sh/loans/"><svg width="30" height="30" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-labelledby="supportIconTitle" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="miter" fill="none" color="#EB4224" class="icon icon-support">
                    <title id="supportIconTitle">Support</title>
                    <path stroke-linecap="round" d="M11 8L9.42229 7.21115C9.14458 7.07229 8.83835 7 8.52786 7H7.82843C7.29799 7 6.78929 7.21071 6.41421 7.58579L5.58579 8.41421C5.21071 8.78929 5 9.29799 5 9.82843L5 14.9296C5 15.5983 5.3342 16.2228 5.8906 16.5937L9.75746 19.1716C10.4944 19.663 11.4668 19.611 12.1472 19.044L17 15"/>
                    <path d="M14.4549 12.9142C13.8515 12.1062 12.741 11.8739 11.8643 12.3721L10.009 13.4266C9.41298 13.7653 8.66412 13.6641 8.17937 13.1794V13.1794C7.54605 12.546 7.59324 11.5056 8.2813 10.9323L12.4437 7.46356C12.8032 7.16403 13.2562 7 13.7241 7H14.5279C14.8384 7 15.1446 7.07229 15.4223 7.21115L17.8944 8.44721C18.572 8.786 19 9.47852 19 10.2361L19 12.9796C19 14.9037 16.5489 15.718 15.3976 14.1764L14.4549 12.9142Z"/>
                    <path d="M1 17V8"/>
                    <path d="M1 17V8"/>
                    <path d="M23 17V8"/>
                </svg><span class="r-suffix--wider r-list-item_text">Loans</span>
            </a>
        </li><li class="r-no-border r-mb-h">
            <a class=" r-list-item is-vertical-nav" href="https://adx-usr.now.sh/investments/"><svg role="img" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" aria-labelledby="thumbUpIconTitle" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="miter" fill="none" color="#EB4224" class="icon icon-thumb-up">
                    <title id="thumbUpIconTitle">Thumb Up</title>
                    <path d="M8,8.73984815 C8,8.26242561 8.17078432,7.80075162 8.4814868,7.43826541 L13.2723931,1.84887469 C13.7000127,1.34998522 14.4122932,1.20614658 15,1.5 C15.5737957,1.78689785 15.849314,2.45205792 15.6464466,3.06066017 L14,8 L18.6035746,8 C18.7235578,8 18.8432976,8.01079693 18.9613454,8.03226018 C20.0480981,8.22985158 20.7689058,9.27101818 20.5713144,10.3577709 L19.2985871,17.3577709 C19.1256814,18.3087523 18.2974196,19 17.3308473,19 L10,19 C8.8954305,19 8,18.1045695 8,17 L8,8.73984815 Z" />
                    <path d="M4,18 L4,9" />
                </svg><span class="r-suffix--wider r-list-item_text">Investments</span>
            </a>
        </li><li class="r-no-border r-mb-h">
            <a class=" r-list-item is-vertical-nav" href="https://adx-usr.now.sh/transactions/"><svg width="30" height="30" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-labelledby="verifiedIconTitle" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="miter" fill="none" color="#EB4224" class="icon icon-verified">
                    <title id="verifiedIconTitle">Verified</title>
                    <path d="M8 12.5L10.5 15L16 9.5" />
                    <path d="M12 22C13.2363 22 14.2979 21.2522 14.7572 20.1843C14.9195 19.8068 15.4558 19.5847 15.8375 19.7368C16.9175 20.1672 18.1969 19.9453 19.0711 19.0711C19.9452 18.1969 20.1671 16.9175 19.7368 15.8376C19.5847 15.4558 19.8068 14.9195 20.1843 14.7572C21.2522 14.2979 22 13.2363 22 12C22 10.7637 21.2522 9.70214 20.1843 9.24282C19.8068 9.08046 19.5847 8.54419 19.7368 8.16246C20.1672 7.08254 19.9453 5.80311 19.0711 4.92894C18.1969 4.05477 16.9175 3.83286 15.8376 4.26321C15.4558 4.41534 14.9195 4.1932 14.7572 3.8157C14.2979 2.74778 13.2363 2 12 2C10.7637 2 9.70214 2.74777 9.24282 3.81569C9.08046 4.19318 8.54419 4.41531 8.16246 4.26319C7.08254 3.83284 5.80311 4.05474 4.92894 4.92891C4.05477 5.80308 3.83286 7.08251 4.26321 8.16243C4.41534 8.54417 4.1932 9.08046 3.8157 9.24282C2.74778 9.70213 2 10.7637 2 12C2 13.2363 2.74777 14.2979 3.81569 14.7572C4.19318 14.9195 4.41531 15.4558 4.26319 15.8375C3.83284 16.9175 4.05474 18.1969 4.92891 19.0711C5.80308 19.9452 7.08251 20.1671 8.16243 19.7368C8.54416 19.5847 9.08046 19.8068 9.24282 20.1843C9.70213 21.2522 10.7637 22 12 22Z" />
                </svg><span class="r-suffix--wider r-list-item_text">Transactions</span>
            </a>
        </li><li class="r-no-border r-mb-h">
            <a class=" r-list-item is-vertical-nav" href="https://adx-usr.now.sh/payment/"><svg role="img" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" aria-labelledby="creditCardIconTitle" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="miter" fill="none" color="#EB4224" class="icon icon-payments">
                    <title id="creditCardIconTitle">Credit Card</title>
                    <rect width="20" height="14" x="2" y="5" rx="2" />
                    <path d="M2,14 L22,14" />
                </svg><span class="r-suffix--wider r-list-item_text">Payment</span>
            </a>
        </li><li class="r-no-border r-mb-h">
            <a class=" r-list-item is-vertical-nav" href="https://adx-usr.now.sh/settings/"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#EB4224" stroke-width="1.875" stroke-linecap="round" stroke-linejoin="round" class="icon icon-settings">
                    <line x1="4" y1="21" x2="4" y2="14" />
                    <line x1="4" y1="10" x2="4" y2="3" />
                    <line x1="12" y1="21" x2="12" y2="12" />
                    <line x1="12" y1="8" x2="12" y2="3" />
                    <line x1="20" y1="21" x2="20" y2="16" />
                    <line x1="20" y1="12" x2="20" y2="3" />
                    <line x1="1" y1="14" x2="7" y2="14" />
                    <line x1="9" y1="8" x2="15" y2="8" />
                    <line x1="17" y1="16" x2="23" y2="16" />
                </svg><span class="r-suffix--wider r-list-item_text">Settings</span>
            </a>
        </li></ul>
</nav>
