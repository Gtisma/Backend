...
@section('content')
    <section class="r-split-twin_content r-gutter--bottom r-d-flex r-flex-dir-col r-al-i-c r-tx-c r-gutter--2x@lg r-edges">
        <h2 class="r-sr">{{ __('Welcome Back') }}</h2>
            ...
            @endif
                ...
                <form method="POST" action="{{ route('password.update') }}" >
                    ...
                    <p class="input-field r-mb-0">
                        ...
                        <input id="email" name="email" class="validate  @error('email') is-invalid @enderror" value="{{ $email ?? old('email') }}" type="email" required autocomplete="email" autofocus>
                    ...
                    <p class="input-field r-mb-0">
                        ...
                        @error('password')
                        <span class="invalid-feedback r-tx-l  r-fs-pico r-red" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        @enderror
                    </p>
                    <p class="input-field r-mb-0">
                        <input id="password" name="password_confirmation" type="password"  required autocomplete="current-password" >
                    ...
                    <button type="submit" class="r-btn r-btn--primary r-btn--match-input r-btn--left-floated-icon input-field r-btn-spinner">
                        ...
                        <span class="r-fw-medium r-btn_text">{{ __('Reset Password') }}</span>
