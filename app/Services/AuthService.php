...
class AuthService
{
    ...
    protected function loginExistingSocialRecord($source,$payload){
        ...
        if($source == Constants::Source[4]){$key="apple_id"; $value = $payload->apple_id; }
    ...
    protected function findSourceid($source,$payload){
        ...
        if($source == Constants::Source[4] && !isset($payload->apple_id)) return ["error"=>"A valid apple id must be sent"];
    }
